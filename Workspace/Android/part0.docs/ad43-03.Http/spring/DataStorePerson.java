...
public final class DataStorePerson {
    ...
    public static ModelPerson loginperson(String id, String pw) {
    ...
    public static ModelPerson selectone(ModelPerson person) {
        logger.info("select " + person);
        synchronized (lists) {

            ModelPerson result = lists.stream()
    ...
    public static List<ModelPerson> selectlist(ModelPerson person) {
        logger.info("select " + person);
        synchronized (lists) {

            List<ModelPerson> result = lists.stream()
                                            .filter( e-> (    e.getId()   .contains(person.getId()    )
                                                           && e.getEmail().contains(person.getEmail() ) 
                                                           && e.getName() .contains(person.getName()  )
                                                           && e.getPw()   .contains(person.getPw()    )  
                                                         )
                                                   )
                                            .collect( Collectors.toList() );
